...
**说明：**
使用烈焰棒攻击另一名玩家，会给予该玩家 5秒的生命恢复2效果，玩家被击中时播放猫咪叫声，有百分之30的概率生成少量经验，并同时增加一点Neko数值（以计分板实现），当这个值大于50时，每一次有百分之 （这个值-50）*2的概率生成一只名字为 {玩家B的名字}的猫崽 、主人为你的小猫，成功后将Neko数值清零
...
Plays a cat sound when the player is hit.
